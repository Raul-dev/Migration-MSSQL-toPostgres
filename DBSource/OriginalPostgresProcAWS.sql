...
CREATE PROCEDURE dbo.sp_filldimdate$tmp(IN par_fromdate TIMESTAMP WITHOUT TIME ZONE, IN par_todate TIMESTAMP WITHOUT TIME ZONE, IN par_culture VARCHAR, IN par_tablename VARCHAR DEFAULT NULL, IN par_isoutput NUMERIC DEFAULT 1, INOUT p_refcur refcursor DEFAULT NULL)
AS 
$BODY$
/*
[7674 - Severity CRITICAL - AWS SCT can't convert the CONCAT_NULL_YIELDS_NULL clause of the SET statement. Convert your source code manually.]
...
CREATE PROCEDURE uts.usp_unittest$tmp(IN par_withoutput NUMERIC DEFAULT 1, IN par_withcleanup INTEGER DEFAULT 1, IN par_testslist VARCHAR DEFAULT '')
AS 
$BODY$
var_TotalTestCount := 3;
par_TestsList := COALESCE(RTRIM(LTRIM(par_TestsList)), '');
